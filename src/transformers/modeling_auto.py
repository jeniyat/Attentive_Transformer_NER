...
from .modeling_bert import BertForTokenClassification_Seg
from .modeling_bert import BertForTokenClassification_Soft_NER
...
MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Seg = OrderedDict(
    [
        (DistilBertConfig, DistilBertForTokenClassification),
        (CamembertConfig, CamembertForTokenClassification),
        (XLMConfig, XLMForTokenClassification),
        (XLMRobertaConfig, XLMRobertaForTokenClassification),
        (RobertaConfig, RobertaForTokenClassification),
        (BertConfig, BertForTokenClassification_Seg),
        (XLNetConfig, XLNetForTokenClassification),
        (AlbertConfig, AlbertForTokenClassification),
        (ElectraConfig, ElectraForTokenClassification),
    ]
...
MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Soft_NER= OrderedDict(
    [
        (DistilBertConfig, DistilBertForTokenClassification),
        (CamembertConfig, CamembertForTokenClassification),
        (XLMConfig, XLMForTokenClassification),
        (XLMRobertaConfig, XLMRobertaForTokenClassification),
        (RobertaConfig, RobertaForTokenClassification),
        (BertConfig, BertForTokenClassification_Soft_NER),
        (XLNetConfig, XLNetForTokenClassification),
        (AlbertConfig, AlbertForTokenClassification),
        (ElectraConfig, ElectraForTokenClassification),
    ]
...
class AutoModelForTokenClassification_Soft_NER:
    r"""
    ...
    def from_config(cls, config):
        ...
        """
        for config_class, model_class in MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Soft_NER.items():
            if isinstance(config, config_class):
                return model_class(config)
        ...
        raise ValueError(
            ...
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Soft_NER.keys()),
            )
        )
...
class AutoModelForTokenClassification_Seg:
    r"""
        :class:`~transformers.AutoModelForTokenClassification` is a generic model class
        that will be instantiated as one of the token classification model classes of the library
        when created with the `AutoModelForTokenClassification.from_pretrained(pretrained_model_name_or_path)`
    ...
    def from_config(cls, config):
        ...
        Examples::
            ...
            model = AutoModelForTokenClassification.from_config(config)  # E.g. model was saved using `save_pretrained('./test/saved_model/')`
        """
        for config_class, model_class in MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Seg.items():
            if isinstance(config, config_class):
                return model_class(config)
        ...
        raise ValueError(
            ...
            "Model type should be one of {}.".format(
                config.__class__,
                cls.__name__,
                ", ".join(c.__name__ for c in MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_Seg.keys()),
            )
        )
